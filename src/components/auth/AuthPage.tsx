...
import { Zap, ShieldAlert, Lock, Mail, User, Building, AlertCircle, Loader2, CheckCircle2, ChevronRight, ArrowLeft } from "lucide-react";
...
SUPABASE_SERVICE_ROLE_KEY=your_service_role_key_here`}
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <button 
        onClick={onBack}
        className="absolute top-8 left-8 flex items-center gap-2 text-xs font-bold text-muted-foreground hover:text-white transition-colors group z-20 cursor-pointer"
      >
        ...
        <form onSubmit={authMode === "login" ? handleLoginSubmit : handleSignUpSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              {authMode === "login" && (
                <button type="button" className="text-[10px] font-bold text-primary hover:text-primary-hover transition-colors cursor-pointer">
                  Forgot Password?
                </button>
              )}
            </div>
